...
var _ = Describe("Save product", func() {
	...
	It("will created product successfully", func() {
		...
		codeRepo.EXPECT().SaveDeployKey(gomock.Any(), gomock.Any(), gomock.Any()).Return(defaultProjectDeployKey, nil)
		codeRepo.EXPECT().ListDeployKeys(gomock.Any(), int(defautlProject.Id), gomock.Any()).Return(listDeployKeys, nil)
		codeRepo.EXPECT().DeleteDeployKey(gomock.Any(), int(defautlProject.Id), int(2014)).Return(nil)

		secretRepo := NewMockSecretrepo(ctl)
		secretRepo.EXPECT().GetDeployKey(gomock.Any(), gomock.Any()).Return(nil, commonv1.ErrorSecretNotFound("secret data is not found"))
		secretRepo.EXPECT().SaveDeployKey(gomock.Any(), gomock.Eq(int(defautlProject.Id)), gomock.Any(), extendKVs).Return(nil)
		secretRepo.EXPECT().AuthorizationSecret(gomock.Any(), gomock.Eq(int(defautlProject.Id)), _ProductDestUser).Return(nil)
	...
	It("will updated product successfully", func() {
		...
		codeRepo.EXPECT().CreateGroup(gomock.Any(), gitOptions).Return(defaultProductGroup, nil)
		codeRepo.EXPECT().GetCodeRepo(gomock.Any(), defaultProjectPath).Return(defautlProject, nil)
		codeRepo.EXPECT().SaveDeployKey(gomock.Any(), gomock.Any(), gomock.Any()).Return(newDefaultProjectDeployKey, nil)
	...
	It("failed to save deploy key", func() {
		...
		codeRepo.EXPECT().SaveDeployKey(gomock.Any(), gomock.Any(), gomock.Any()).Return(nil, fmt.Errorf("unable to add deploy key"))
		codeRepo.EXPECT().GetCurrentUser(gomock.Any()).Return(_GitUser, _GitEmail, nil)
	...
	It("failed to save secret", func() {
		...
		secretRepo := NewMockSecretrepo(ctl)
		secretRepo.EXPECT().GetDeployKey(gomock.Any(), gomock.Any()).Return(nil, commonv1.ErrorSecretNotFound("secret data is not found"))
		secretRepo.EXPECT().SaveDeployKey(gomock.Any(), gomock.Eq(int(defautlProject.Id)), gomock.Any(), extendKVs).Return(fmt.Errorf("unable to save secret"))
	...
	It("failed to authorization secret", func() {
		...
		secretRepo.EXPECT().GetDeployKey(gomock.Any(), gomock.Any()).Return(nil, commonv1.ErrorSecretNotFound("secret data is not found"))
		secretRepo.EXPECT().SaveDeployKey(gomock.Any(), gomock.Eq(int(defautlProject.Id)), gomock.Any(), extendKVs).Return(nil)
		secretRepo.EXPECT().AuthorizationSecret(gomock.Any(), gomock.Eq(int(defautlProject.Id)), _ProductDestUser).Return(fmt.Errorf("Failed to authorization"))
